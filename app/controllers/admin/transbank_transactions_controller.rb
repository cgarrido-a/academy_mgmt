module Admin
  class TransbankTransactionsController < Admin::ApplicationController
    ...
    def index
      @transactions = TransbankTransaction.includes(:enrollment, :installment, enrollment: { student: :user })
    ...
    def set_transaction
      @transaction = TransbankTransaction.includes(:enrollment, :installment, enrollment: { student: :user }).find(params[:id])
    end
  end
end
